...
async function main() {
  ...
  try {
    ...
    assert.equal(conversation.promptVersion, 'bazi-chat-annual-timeline-v4');
    ...
    assert.deepEqual(built.manifest.allowedCapabilities, [
      'strength_evidence_audit', 'pattern_candidates', 'useful_god_method_separation', 'luck_cycle_schedule', 'annual_timeline_schedule',
    ]);
