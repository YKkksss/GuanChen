...
export const BAZI_CHAT_PROMPT_VERSION = 'bazi-chat-month-day-timeline-v13';
...
export function createBaziConversation(input: {
  ...
  `).run(
    id, chart.id, analysis.id, luckCycles.id, annualTimeline.id, relationAudit.id, relationAdjudication.id, dynamicTenGod.id, tenGodRepeat.id, transparencyRoot.id, hiddenStemActivation.id, strengthComposite.id, patternCondition.id, monthDayTimeline.id, title,
...
export function getBaziConversation(id: string): BaziConversationDetail | null {
  ...
  if (!row) return null;
  if (!row.analysis_version_id || !row.luck_cycle_version_id || !row.annual_timeline_version_id || !row.relation_audit_version_id || !row.relation_adjudication_version_id || !row.dynamic_ten_god_version_id || !row.ten_god_repeat_version_id || !row.transparency_root_version_id || !row.hidden_stem_activation_version_id || !row.strength_composite_version_id || !row.pattern_condition_version_id || !row.month_day_timeline_version_id || row.prompt_version !== BAZI_CHAT_PROMPT_VERSION) {
    ...
    getDatabase().prepare(`
      ...
      SET analysis_version_id = ?, luck_cycle_version_id = ?, annual_timeline_version_id = ?, relation_audit_version_id = ?,
          relation_adjudication_version_id = ?, dynamic_ten_god_version_id = ?, ten_god_repeat_version_id = ?, transparency_root_version_id = ?, hidden_stem_activation_version_id = ?, strength_composite_version_id = ?, pattern_condition_version_id = ?, month_day_timeline_version_id = ?, prompt_version = ?, updated_at = ?
      WHERE id = ?
    `).run(analysis.id, luckCycles.id, annualTimeline.id, relationAudit.id, relationAdjudication.id, dynamicTenGod.id, tenGodRepeat.id, transparencyRoot.id, hiddenStemActivation.id, strengthComposite.id, patternCondition.id, monthDayTimeline.id, BAZI_CHAT_PROMPT_VERSION, Date.now(), id);
  ...
  const monthDayTimeline = conversation.monthDayTimelineVersionId
    ...
    : null;
  return { ...conversation, chart, profile, analysis, luckCycles, annualTimeline, relationAudit, relationAdjudication, dynamicTenGod, tenGodRepeat, transparencyRoot, hiddenStemActivation, strengthComposite, patternCondition, monthDayTimeline };
}
