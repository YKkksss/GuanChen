...
项目名称为 **观辰**，功能说明为 **命盘与人生观察**。品牌标识采用朱砂色星轨与四芒星图形。从命盘、对话到人生事件，建立一份可以持续回看的个人档案。安装默认使用 `main` 分支；原版 README.md 保留作历史说明。

## 界面预览

以下为观辰当前开发版本的实际页面截图，桌面视口为 1440 × 1000。命盘使用合成出生资料，聊天和报告使用人工编写的展示内容，不包含真实用户档案，也不代表真实 AI 生成结果。点击图片可查看原图。
...
| 合盘 | 双命盘与关系背景、互动结构分析、关系报告及阶段观察 |
| 八字 | 排盘、历史资料、AI 解读、大运及年/月/日相关分析 |
...
1. 持续核对命盘 AI 解读的上下文准确性和连续追问效果。
2. 验收已确认事件与运限分析的关联体验。
3. **继续专题报告事实组织**：当前大限年龄口径、专题相关事件筛选、事实缺失提示与信息价值。
4. 按顺序完善合盘、八字、生时校正的实际业务流程。
5. 核心功能达到固定范围后，完成必要恢复验证、完整使用流程、桌面、真实手机及另一台局域网主机验收，再确认冻结版本。
