...
M2-4 已增加自然年份／虚岁年龄双轴：两种口径复用同一批事件事实，支持本地记忆展示偏好、筛选后跨度摘要、年份或年龄快速定位，以及日期范围的起止虚岁展示。实现细节见 `docs/M2_4_EVENT_DUAL_AXIS_IMPLEMENTATION.md`。
...
命盘页和运限分析页均提供入口。事件卡展示分类、日期、影响程度，以及按层级分组的流年、流月、流日摘要；点击某个关联可进入对应时间层级。时间轴可在自然年份和虚岁年龄之间切换，并可快速定位到当前筛选结果中的任一节点。
...
测试覆盖日期校验、CRUD、年月日关联、区间起止边界、修改后重建关联、筛选、数据库迁移、级联删除，以及年份／虚岁换算、同年聚合、区间年龄和日期不详节点。
...
1. 自然年份与虚岁年龄双轴切换；
2. 当前筛选结果的时间跨度摘要与节点快速定位；
3. 区间事件的起止虚岁展示和日期不详安全降级。

后续增强：更完整的本地档案导入、备份和恢复。
