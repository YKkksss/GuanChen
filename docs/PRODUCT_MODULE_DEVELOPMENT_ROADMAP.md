...
当前最近的开发目标是 M3 报告增强中的“服务端 PDF 文件导出与报告版本差异比较”。M2-1 已完成聊天候选事件智能提取、用户核对确认、正式事件与年度关联、确认记忆和上下文注入，并从数据层保证 AI 候选不能直接成为现实事实。下一步让已保存的专题报告、年度报告、合盘报告与校时报告具备一致的服务端导出能力，再增加同类型报告版本间的章节和证据差异视图；M10 独立术数产品线最后单独立项。
