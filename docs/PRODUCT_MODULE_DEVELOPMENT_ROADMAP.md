...
> 实施状态（2026-08-29）：M9-0 至 M9-5 已完成。当前已具备确定性四柱基础、SQLite v23 出生档案与多版本命盘、SQLite v24 独立八字会话、SQLite v25 旺衰格局用神证据版本、SQLite v26 大运排期版本、SQLite v27 流年时间轴版本、无伪精确分数的旺衰证据审计、月令格局候选、四类取用方法隔离、结构冲合复核、分钟精度交运时间轴、精确立春切年、出生和末运裁剪、流年与大运区间求交、年中交运双片段、未知时辰与未校准时区降级，以及包含流年排期事实的独立聊天工作台。下一阶段进入 M9-6 原局—大运—流年作用关系证据审计；当前仍不开放大运吉凶、流年吉凶和具体事件预测。
