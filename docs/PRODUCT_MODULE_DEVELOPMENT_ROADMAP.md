...
> 实施状态（2026-08-31）：第一版年度确定性快照、SQLite 缓存、年度分析页、AI 年份上下文及只读年度总结报告已完成；M1-2 流月确定性快照、农历月界、十二宫与流曜映射、缓存、时间页面和 AI 流月上下文已完成；M1-3 流日确定性快照、早子时日期边界、逐日缓存、日期导航与 AI 流日上下文也已完成。全年十二流月时间轴、重点月份对比和现实事件精确挂接仍待开发。实现细节见 `docs/M1_ANNUAL_TRANSIT_IMPLEMENTATION.md`、`docs/M1_2_MONTHLY_TRANSIT_IMPLEMENTATION.md` 与 `docs/M1_3_DAILY_TRANSIT_IMPLEMENTATION.md`。
...
> 实施状态（2026-08-30）：M2 第一版与 M2-1 已完成。当前支持手动事件 CRUD、日期精度、分类筛选、JSON 导出、年度运势自动关联、时间轴，以及聊天事件智能提取、独立候选层、用户核对确认、忽略与幂等处理、确认后正式事件／年度关联／记忆同步、按问题和年份注入已确认事件上下文。SQLite 已升级至 v41，未确认候选不会进入正式事件、记忆、摘要或模型事实。事件专项 AI 回溯、年龄轴切换和批量导入仍待后续增强。实现细节见 `docs/M2_LIFE_EVENTS_IMPLEMENTATION.md` 与 `docs/M2_1_LIFE_EVENT_CANDIDATE_CONFIRMATION_IMPLEMENTATION.md`。
...
当前 M1-3“紫微流日确定性排盘第一版”已经完成：系统能按公历自然日生成早子时代表盘，保存流日十二宫、三层四化、流曜、重点宫位与证据快照，并将流日事实按“大限—流年—流月—流日”注入 AI 上下文。下一步进入 M1-4 全年十二流月时间轴与重点月份对比；之后再补人生事件精确挂接与公共数据能力。M10 独立术数产品线最后单独立项。
