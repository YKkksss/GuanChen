...
> 实施状态（2026-08-29）：M9-0 至 M9-4 已完成。当前已具备确定性四柱基础、SQLite v23 出生档案与多版本命盘、SQLite v24 独立八字会话、SQLite v25 旺衰格局用神证据版本、SQLite v26 大运排期版本、无伪精确分数的旺衰证据审计、月令格局候选、四类取用方法隔离、结构冲合复核、顺逆与前后节审计、分钟精度起运折算、交运日期、八步十年大运时间轴、未知时辰与未校准时区降级，以及包含大运排期事实的独立聊天工作台。下一阶段进入 M9-5 流年确定性时间轴；当前仍不开放大运吉凶、流年吉凶和具体事件预测。
