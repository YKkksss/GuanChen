...
> 2026-09-07：v1.0 范围已由用户确认，以 [v1 固定范围与收尾顺序](./V1_FREEZE_SCOPE.md) 为当前执行依据。按 F13–F15 → F16–F18 → 文案与核心可用性 → 阻断问题 → 发布验收推进，冻结前不扩展新模块。

> 最新用户决策：优先命盘与 AI 解读 → 事件与运势 → 专题报告 → 合盘/八字/校时功能。暂停备份扩展，原发布验收安排后移；以 V1_FREEZE_SCOPE.md 的“当前优先级”为准。

> 前期进展：F13–F18、第三批文案与可用性、第四批阻断修复已交付，下一阶段为独立数据完整流程、备份恢复及实机发布验收。下方 09-05 状态保留为历史记录。
...
> 当前执行状态（2026-09-05）：M0–M9 主线及后续回补基线见 [09-05 AI 交接文档](./AI交接文档/交接文档-2026-09-05.md)。目前推进前端体验审查后的分批修复，F01–F12 已完成，聊天生命周期见 [F11–F12 实施记录](./F11_F12_CHAT_LIFECYCLE_IMPLEMENTATION.md)。用户追加的命盘聊天阅读优化已完成：默认加宽、可拖动分栏与比例记忆、专注对话、事件候选折叠、紧凑输入自适应，以及阅读/输入/行为设置的即时预览和本地保存，详见 [阅读体验实施记录](./CHART_CHAT_READING_EXPERIENCE_IMPLEMENTATION.md)。SQLite 仍为 v47，下一批为 F13 + F14 + F15，尚未立项 M10。下文较早阶段的“下一步”保留作历史规划，不覆盖此处最新执行顺序。

---

## 1. 文档目的

2026-09-05 追加交付：AI 聊天 Markdown 完整渲染、中文强调兼容、流式/中断格式与表格代码自适应已完成，见 [Markdown 实施记录](./AI_CHAT_MARKDOWN_IMPLEMENTATION.md)。下一批仍按上方 F13–F15 顺序执行。
...
> 实施状态（2026-09-01）：第一版年度确定性快照、SQLite 缓存、年度分析页、AI 年份上下文及只读年度总结报告已完成；M1-2 流月确定性快照、农历月界、十二宫与流曜映射、缓存、时间页面和 AI 流月上下文已完成；M1-3 流日确定性快照、早子时日期边界、逐日缓存、日期导航与 AI 流日上下文已完成；M1-4 全年 12／13 流月时间轴、闰月处理、用户选择的重点月份事实对比也已完成。现实事件与流月、流日精确挂接仍待开发。实现细节见 `docs/M1_ANNUAL_TRANSIT_IMPLEMENTATION.md`、`docs/M1_2_MONTHLY_TRANSIT_IMPLEMENTATION.md`、`docs/M1_3_DAILY_TRANSIT_IMPLEMENTATION.md` 与 `docs/M1_4_MONTHLY_YEAR_TIMELINE_IMPLEMENTATION.md`。
...
- [x] 流月宫位、四化、流曜和三方四正规则。
- [x] 流月主题上下文与非绝对化风险边界。
- [x] 按农历流年展开的 12／13 流月时间轴及闰月处理。
- [x] 用户选择两个重点月份进行四化与宫位事实对比。
- [x] 禁止无规则依据的月份评分、吉凶排名和最佳月份推荐。

#### 第三版：流日分析

- [x] 公历日期选择、前后日导航及 URL 状态同步。
- [x] 流日宫位、干支、四化、流曜和三方四正结构。
- [x] 早子时代表盘、晚子时边界说明和独立引擎版本。
- [x] 适合用于日记式观察，不输出绝对吉凶。
- [x] 流日确定性事实进入 AI 四层时间上下文。
- 支持将现实事件记录到当天。
...
> 实施状态（2026-09-01）：M2 第一版至 M2-8 已完成。除事件记录、聊天候选确认、年月日精确挂接、专项回溯和年份／虚岁双轴外，现已具备 v46 整库一致性备份、安全恢复、单命盘可移植副本导入、周期自动备份、保留策略、健康检查与密码保护的认证加密备份。`.ziweibackupx` 使用 scrypt 与 AES-256-GCM，密码不落库，解密后复用完整内层预检。实现细节见 `docs/M2_LIFE_EVENTS_IMPLEMENTATION.md` 至 `docs/M2_8_ENCRYPTED_BACKUP_IMPLEMENTATION.md`。
...
> 实施状态（2026-08-31）：第一版已完成单人命盘七类专题报告、版本历史、结构化依据和失败回退；M3-1 已完成专题、年度、合盘、校时四类报告的统一服务端 PDF；M3-2 已完成四类报告的结构化版本差异比较、生成来源审计，并将年度报告升级为不可变版本历史；M3-3 已完成四类报告按版本隔离的确认状态、个人备注、人工修订副本、原文恢复及修订稿 PDF 导出。实现细节见 `docs/M3_1_SERVER_PDF_EXPORT_IMPLEMENTATION.md`、`docs/M3_2_REPORT_VERSION_COMPARISON_IMPLEMENTATION.md` 与 `docs/M3_3_REPORT_CONFIRMATION_AND_EDITING_IMPLEMENTATION.md`。
...
> 实施状态（2026-08-15）：M4-0 至 M4-5 已完成。现已支持双盘事实隔离、可审计规则、持久化多轮对话、关系工作台、合盘关系报告，以及可缓存、可切换年份、可进入年度专属对话的双人运限。下一步进入 M5 出生时辰校正。
...
- AI 不会把甲方事实写到乙方名下。
- 可以生成和重新打开合盘报告。
- 可以切换年份查看甲乙双方运限、关系维度激活与分层规则结果。
- 同年结果可持久化复用，关系输入变化后自动重建。

---

## 12. M5：出生时辰校正

> 实施状态（2026-08-16）：M5-0 至 M5-5 已完成。现已具备时间换算、候选命盘、事件证据矩阵、可审计规则评分、候选比较与人工选定、结论报告版本，以及从选定记录幂等创建工作命盘的完整闭环。M5 阶段完成，下一步进入 M6 紫微学习模式。
...
相对证据指数不应由 AI 直接决定，也不能解释为真实时辰的正确概率。建议包含：
...
- 同一事件和同一事件分类贡献封顶。
- 没有命中默认中性，不自动视为冲突。
- 至少 4 个事件时执行留一事件稳定性检查。
- 候选差距不足时并列展示，不强行确定第一名。

### 12.5 候选比较页面

- 候选时辰列表。
- 每个候选的命宫、身宫、主星和相对证据指数。
- 事件逐条对照矩阵。
- 支持依据与冲突依据。
- 两个候选命盘差异高亮。
- “设为当前命盘”按钮。
...
> 实施状态（2026-08-22）：M7-0 至 M7-2 已完成。当前已具备匿名案例 SQLite v18 数据基础、脱敏预览、用途授权、复核管理和安全导出，SQLite v19 匿名结构索引与教学检索，以及 SQLite v20 对比持久化、双命盘十维并列比较、同案例双大限七维比较、三态结果、证据路径、来源更新自动重算和授权撤销即时失效。M7 闭环已经完成，下一步进入 M8 主动提醒与周期复盘。
...
> 实施状态（2026-08-23）：M8-0 至 M8-2 已完成，M8 核心闭环完成。当前已具备 SQLite v21 提醒规则与实例、SQLite v22 月度复盘、五类提醒规则、本地提醒中心、首页到期徽标、五段式复盘、预测与现实隔离、用户确认后可选沉淀人生事件和长期记忆，以及来源提醒自动完成。下一大阶段进入 M9 八字体系，优先开展 M9-0 方法论和确定性算法边界设计。
...
> 实施状态（2026-08-30）：M9-0 至 M9-18 已完成，M9 基础审计闭环完成。当前已具备确定性四柱基础、SQLite v23 至 v40 的出生档案、会话、解释证据、大运、流年、关系条件、动态十神、显隐重复、透干通根、藏干触达、月令旺衰综合矩阵、格局条件审计、精确流月流日时间轴、指定日期五层关系、显隐触达、三层方向矩阵，以及按精确片段拆分的“静态格局状态—岁运既有角色—流月流日新增角色—合并覆盖—关系与藏干上下文”映射。M9-18 原样保留 M9-13 候选、检查、静态状态及风险救应链接，动态层只增加可追溯入口，不开放最终旺衰、格局成败、救应完成、用神喜忌、吉凶和具体事件预测。M9 完成后已回补 M2-1 聊天候选事件提取与用户确认，下一阶段进入 M3 报告增强。
...
- 版本不兼容时阻止导入并说明原因。
- 应用打开时按周期补做自动备份。
- 周期备份保留策略、完整健康检查与受保护删除。
- 手动备份、周期备份和恢复前保险副本分级管理。
- 使用本地密码生成可安全复制到外部介质的认证加密备份。
...
当前 M1 年度、流月、流日和全年流月浏览，M2 人生事件与本地数据安全，及 M3 至 M9 的既定核心闭环均已完成。本地档案现已具备整库恢复、单命盘迁移、周期自动备份、保留策略、健康验证和密码保护的认证加密导出。当前产品增强主线已切换到移动端网页版适配，第一阶段已完成首页、起盘、命盘工作台与全局导航骨架，第二阶段已完成手机和平板留白体系、命盘功能抽屉、聚焦视图及十二宫总览/放大阅读模式。后续继续逐模块深化合盘、八字结果、报告、时间轴、校时、学习和数据管理页面。移动端完成后，可在用户明确选择远端服务并授权后接入跨设备同步，也可以进入 M10 独立术数产品线；不在缺少规则验证时提前混合多个术数体系。
