  ...
  return (
    <main className="case-form mx-auto min-h-screen max-w-[1180px] px-4 py-7 sm:px-6 sm:py-10">
      <header className="mb-7 flex flex-wrap items-start justify-between gap-4">
        ...
        <button type="button" onClick={() => setFormOpen(true)} className="flex items-center gap-2 rounded-lg px-5 py-3 text-xs" style={{ color: '#fff8e8', background: 'linear-gradient(135deg,#9a6210,#c88020)' }}><Plus size={14} />新建提醒</button>
      </header>
...
function ReminderRuleForm({ conversations, onClose, onCreated }: { conversations: ConversationListItem[]; onClose: () => void; onCreated: () => Promise<void> }) {
    ...
    try {
      ...
      const response = await fetch('/api/reminders', {
        ...
        body: JSON.stringify({
          ...
          conversationId: needsConversation(kind) ? conversationId : null,
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-5"><section role="dialog" aria-modal="true" aria-labelledby="reminder-form-title" className="max-h-[92vh] w-full max-w-[720px] overflow-y-auto rounded-t-2xl p-5 sm:rounded-2xl sm:p-6" style={{ background: 'var(--t-bg)', border: '1px solid var(--t-border-acc)' }}><div className="flex items-start justify-between gap-4"><div><div className="text-[9px] tracking-[.22em]" style={{ color: 'var(--t-gold)' }}>NEW LOCAL REMINDER</div><h2 id="reminder-form-title" className="mt-1 text-lg font-semibold" style={{ color: 'var(--t-text)' }}>新建本地提醒</h2><p className="mt-2 text-[10px] leading-5" style={{ color: 'var(--t-faint)' }}>提醒只保存在这台设备的 SQLite 中，应用未运行时不会弹出通知。</p></div><button type="button" aria-label="关闭新建提醒" onClick={onClose} className="rounded-lg p-2" style={{ color: 'var(--t-faint)', border: '1px solid var(--t-border)' }}><X size={15} /></button></div>
    <form onSubmit={submit} className="mt-6 space-y-5">
      <div className="grid gap-2 sm:grid-cols-5">{(Object.keys(KIND_LABELS) as ReminderKind[]).map(value => <button key={value} type="button" onClick={() => changeKind(value)} className="rounded-lg px-3 py-3 text-[10px]" style={kind === value ? { color: '#fff8e8', background: '#9a6210' } : { color: 'var(--t-text2)', border: '1px solid var(--t-border)' }}>{KIND_LABELS[value]}</button>)}</div>
      ...
      <div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg px-5 py-3 text-xs" style={{ color: 'var(--t-text2)', border: '1px solid var(--t-border)' }}>取消</button><button type="submit" disabled={saving} className="rounded-lg px-5 py-3 text-xs disabled:opacity-40" style={{ color: '#fff8e8', background: '#9a6210' }}>{saving ? '正在生成提醒…' : '保存并生成提醒'}</button></div>
    </form>
  </section></div>;
}
...
function ViewButton({ active, onClick, title, subtitle }: { active: boolean; onClick: () => void; title: string; subtitle: string }) { return <button type="button" onClick={onClick} className="flex-1 rounded-lg px-4 py-3 text-left transition" style={active ? { color: '#fff8e8', background: '#9a6210' } : { color: 'var(--t-text2)' }}><span className="block text-xs font-medium">{title}</span><span className="mt-1 block text-[9px] opacity-70">{subtitle}</span></button>; }
...
function needsConversation(kind: ReminderKind) { return kind === 'birthday_review' || kind === 'transit_change' || kind === 'event_anniversary'; }
