  ...
  return (
    <main className="case-form mx-auto min-h-screen max-w-[1280px] px-4 py-7 sm:px-6 sm:py-10">
      ...
      <section className="mb-6 grid gap-3 rounded-xl card-glass p-4 sm:grid-cols-[1fr_180px_auto] sm:items-end">
        ...
        <button type="button" disabled={saving || loading || !conversations.length} onClick={() => void startReview()} className="flex min-h-11 items-center justify-center gap-2 rounded-lg px-5 text-xs disabled:opacity-40" style={{ color: '#fff8e8', background: '#9a6210' }}><Plus size={14} />开始或继续复盘</button>
      </section>
...
function ConfirmReviewDialog({ review, saving, onClose, onConfirm }: {
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-5"><section role="dialog" aria-modal="true" aria-labelledby="confirm-review-title" className="max-h-[92vh] w-full max-w-[640px] overflow-y-auto rounded-t-2xl p-5 sm:rounded-2xl sm:p-6" style={{ background: 'var(--t-bg)', border: '1px solid var(--t-border-acc)' }}><div className="flex items-start justify-between gap-4"><div><div className="text-[9px] tracking-[.22em]" style={{ color: 'var(--t-gold)' }}>USER CONFIRMATION</div><h2 id="confirm-review-title" className="mt-2 text-lg font-semibold" style={{ color: 'var(--t-text)' }}>确认 {monthLabel(review.reviewMonth)}复盘</h2><p className="mt-2 text-[10px] leading-5" style={{ color: 'var(--t-faint)' }}>确认后复盘内容会锁定，并自动完成关联提醒。以下沉淀操作只执行你明确勾选的项目。</p></div><button type="button" aria-label="关闭确认复盘" onClick={onClose} className="rounded-lg p-2" style={{ color: 'var(--t-faint)', border: '1px solid var(--t-border)' }}><X size={15} /></button></div>
    ...
    <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg px-5 py-3 text-xs" style={{ color: 'var(--t-text2)', border: '1px solid var(--t-border)' }}>返回修改</button><button type="button" disabled={!canSubmit} onClick={() => void onConfirm({ saveToMemory, event: createEvent ? { title: eventTitle, category: eventCategory, impactLevel } : null })} className="rounded-lg px-5 py-3 text-xs disabled:opacity-40" style={{ color: '#fff8e8', background: '#9a6210' }}>{saving ? '正在确认…' : '确认并锁定'}</button></div>
  </section></div>;
}
...
function ScoreField({ item, value, onChange }: { item: typeof DIMENSIONS[number]; value: number | null; onChange: (value: number | null) => void }) {
  return <div className="rounded-xl p-3" style={{ background: 'var(--ac-bg)', border: '1px solid var(--t-border)' }}><div className="flex items-start justify-between"><div><div className="text-xs" style={{ color: 'var(--t-text)' }}>{item.label}</div><div className="mt-1 text-[8px]" style={{ color: 'var(--t-faint)' }}>{item.hint}</div></div>{value && <button type="button" onClick={() => onChange(null)} className="text-[8px]" style={{ color: 'var(--t-faint)' }}>清空</button>}</div><div className="mt-3 grid grid-cols-5 gap-1">{[1, 2, 3, 4, 5].map(score => <button key={score} type="button" onClick={() => onChange(score)} className="rounded-md py-2 text-[10px]" style={value === score ? { color: '#fff8e8', background: '#9a6210' } : { color: 'var(--t-text2)', border: '1px solid var(--t-border)' }}>{score}</button>)}</div></div>;
}
