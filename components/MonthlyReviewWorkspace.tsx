...
function ConfirmReviewDialog({ review, saving, onClose, onConfirm }: {
  ...
  return <div ref={dialogRef} tabIndex={-1} className="v1-dialog fixed inset-0 z-[80] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-5"><section role="dialog" aria-modal="true" aria-labelledby="confirm-review-title" className="max-h-[92vh] w-full max-w-[640px] overflow-y-auto rounded-t-2xl p-5 sm:rounded-2xl sm:p-6" style={{ background: 'var(--t-bg)', border: '1px solid var(--t-border-acc)' }}><div className="flex items-start justify-between gap-4"><div><div className="text-[9px] tracking-[.22em]" style={{ color: 'var(--t-gold)' }}>USER CONFIRMATION</div><h2 id="confirm-review-title" className="mt-2 text-lg font-semibold" style={{ color: 'var(--t-text)' }}>确认 {monthLabel(review.reviewMonth)}复盘</h2><p className="mt-2 text-[10px] leading-5" style={{ color: 'var(--t-faint)' }}>确认后复盘内容会锁定，并自动完成关联提醒。以下沉淀操作只执行你明确勾选的项目。</p></div><button type="button" aria-label="关闭确认复盘" disabled={saving} onClick={closeDialog} className="rounded-lg p-2" style={{ color: 'var(--t-faint)', border: '1px solid var(--t-border)' }}><X size={15} /></button></div>
    ...
    <div className="mt-5 rounded-lg p-3 text-[10px] leading-5" style={{ color: 'var(--t-text2)', background: 'var(--ac-bg)', border: '1px solid var(--t-border)' }}>未勾选的内容不会进入人生事件或长期记忆。复盘原文仍会作为已确认记录保存在部署电脑的数据库中。</div>
    <div className="mt-6 flex justify-end gap-2"><button type="button" disabled={saving} onClick={closeDialog} className="rounded-lg px-5 py-3 text-xs" style={{ color: 'var(--t-text2)', border: '1px solid var(--t-border)' }}>返回修改</button><button type="button" disabled={!canSubmit} onClick={() => void onConfirm({ saveToMemory, event: createEvent ? { title: eventTitle, category: eventCategory, impactLevel } : null })} className="rounded-lg px-5 py-3 text-xs disabled:opacity-40" style={{ color: '#fffaf3', background: 'var(--ac)' }}>{saving ? '正在确认…' : '确认并锁定'}</button></div>
  </section></div>;
}
