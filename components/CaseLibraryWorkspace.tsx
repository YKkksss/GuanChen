  ...
  return (
    <main className="case-form mx-auto min-h-screen max-w-[1220px] px-4 py-7 sm:px-6 sm:py-10">
      ...
      {view === 'teaching' ? (
        ...
        <section className="mb-6 rounded-xl card-glass p-4"><div className="flex flex-wrap gap-2">{STATUS_OPTIONS.map(option => <button key={option.value} type="button" onClick={() => setStatus(option.value)} className="rounded-lg px-3.5 py-2 text-[11px] transition" style={status === option.value ? { color: '#fff8e8', background: '#9a6210' } : { color: 'var(--t-text2)', border: '1px solid var(--t-border)' }}>{option.label}</button>)}</div></section>
      )}
