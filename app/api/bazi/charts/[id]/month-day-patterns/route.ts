...
import {
  ...
  getBaziMonthDayPatternForChartDate,
  listBaziMonthDayPatternVersions,
} from '@/lib/db/bazi-month-day-patterns';
...
export async function GET(request: Request, context: RouteContext) {
  ...
  return NextResponse.json({
    monthDayPattern: targetDate ? getBaziMonthDayPatternForChartDate(id, targetDate) : null,
    versions: listBaziMonthDayPatternVersions(id),
  });
}
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json({
      monthDayPattern: ensureBaziMonthDayPatternVersion(id, targetDate, targetYear),
    }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : '八字流月流日格局条件映射生成失败';
