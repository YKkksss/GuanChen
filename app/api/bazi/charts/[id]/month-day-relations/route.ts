...
import {
  ...
  getBaziMonthDayRelationForChartDate,
  listBaziMonthDayRelationVersions,
...
export async function GET(request: Request, context: RouteContext) {
  ...
  return NextResponse.json({
    monthDayRelation: targetDate ? getBaziMonthDayRelationForChartDate(id, targetDate) : null,
    versions: listBaziMonthDayRelationVersions(id),
  });
}
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json({
      monthDayRelation: ensureBaziMonthDayRelationVersion(id, targetDate, targetYear),
    }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : '八字流月流日动态关系生成失败';
